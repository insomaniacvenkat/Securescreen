...
ASCII diagram of the app flow:
  ...
  +------------+--------------+
               ...
               v
  +---------------------------+
  | App stores persistable    |
  | permission and dispatches |
  | to viewer:               |
  +------------+--------------+
               |
       +-------+--------+
       ...
       v                v
  [Image flow]       [PDF flow]
   decode stream      open ParcelFileDescriptor
   -> Bitmap          -> PdfRenderer
   -> show in         -> render pages to Bitmap(s)
      ZoomableImageView   -> ViewPager2 + adapter

Cross-cutting:
- On every Activity onCreate/onResume, MyApp applies FLAG_SECURE so system blocks screenshots/recording.

Files of interest
-----------------
- app/src/main/java/com/example/securescreen/MyApp.java
- app/src/main/java/com/example/securescreen/MainActivity.java
- app/src/main/java/com/example/securescreen/PdfPagerAdapter.java
- app/src/main/java/com/example/securescreen/ZoomableImageView.java
